...
**Need-based push limit (v0.88.0):** The artificial `push_receive_headroom` cap (v0.64.0, tuned 1.15→2.0→5.0) was replaced with a deployment-target-based room check. Push quantity per DC is capped at remaining room below ABC-differentiated deployment target (A=10.5, B=14.0, C=17.5 DOS) — same targets used by `_compute_deployment_needs()`. This closes the positive feedback loop where uncapped push → rising DC inventory → reduced pull → rising RDC excess → more push. Room check uses on-hand inventory (not IP): simpler, self-correcting at daily granularity.

**DRP Distribution — Shipping Disabled by Default (v0.89.1):** DRP distribution engine (`drp_distribution.py`) always initialized to provide demand signals for priming and deployment, but RDC→DC shipping gated by `_drp_ships` flag (= `config.drp.enabled`, default `false`). Pure DRP suffers product-mix mismatch with (s,S) store batching (72.6% fill rate). DC pull only: 94.6%. Hybrid (DRP+DC pull): 92.9%. `_compute_dc_need()` uses replenisher's rolling-avg DC outflow when available (floors at base POS). Config: `dc_target_amplification` for bullwhip buffer, `suppress_dc_pull` for hybrid mode.
...
**Variable BOM Depth (v0.84.0):** Extended 3-level BOM to N-level (2-4 levels) with variable depth across portfolio. ~70% SKUs stay 2-level, ~20% get 3-level depth via premix sub-intermediates (PREMIX-* products, bom_level=2, BULK_INTERMEDIATE category), ~10% get multi-intermediate (2 bulk refs per SKU: primary 0.7 + secondary blend 0.3). Total products: ~640 (was 623). Diamond dependencies arise naturally from shared raw materials between premix and direct bulk inputs. MRP refactored from 2-step to N-step level-by-level explosion (ascending through intermediate levels). TransformEngine unchanged (already sorts by -bom_level). ERP generator updated to use category-based detection instead of prefix matching.

**Multi-Source DC Deployment Fix (v0.85.0):** Made deployment shares, expected demand vectors, and push mechanism aware of secondary-source fraction. v0.83.0's multi-source DCs ordered 20% from secondary RDCs, but deployment/push only saw primary-tree demand — primary RDCs accumulated (34 DOS), secondary RDCs depleted. Fix shifts `frac × dc_demand` from primary to secondary targets in all three systems (`_calculate_deployment_shares`, `_precompute_deployment_targets`, `_push_excess_rdc_inventory`). Zero-sum: total demand unchanged. 4 DCs mapped (1 RDC-primary + 3 plant-direct). All changes in `orchestrator.py`.
