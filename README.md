# Prism Consumer Goods — Synthetic Enterprise Data Generator

A physics-based discrete-event simulation that generates ~395 million rows of synthetic ERP data for a fictional $38.3 billion FMCG company.

**This is a generator, not a dataset download.** The output is approximately 40 GB of parquet and CSV files — too large for git. You clone the repo, run the pipeline, and produce the data on your machine.

The generator produces 38 normalized ERP tables covering procurement, manufacturing, order fulfillment, returns, and financial accounting. Every row traces back to a simulation that enforces mass balance, capacity constraints, and kinematic consistency. The financial layer is generated post-hoc from operational data — the same way real ERP systems record transactions after physical events occur.

**Purpose-built for:**

- **Virtual Knowledge Graph (VKG) testing** — heterogeneous enterprise tables that must be integrated and queried as a unified graph
- **Supply chain analytics** — realistic volume, variability, and cross-functional tradeoffs across ~4,200 network nodes
- **Data integration benchmarking** — controlled data quality issues with a known ground truth for entity resolution, record matching, and anomaly detection

**Pipeline overview:**

```
install → generate world → stabilize → simulate → generate ERP → load PostgreSQL
```
...
**Prerequisites:** Python 3.12+, [Poetry](https://python-poetry.org/), PostgreSQL 16+, ~16 GB RAM, ~50 GB disk.
...
# 2. Generate static world (~4,200 nodes, <1 min)
poetry run python scripts/generate_static_world.py

# 3. Stabilization run — 3 years, builds converged inventory state (~20 min)
poetry run python run_simulation.py --days 1095 --no-logging --snapshot

# 4. Production run — 365 days from converged state (~25 min)
...
**Supply chain diagnostic** (runs on parquet output, 35 questions, 8 sections):

```bash
poetry run python scripts/analysis/diagnose_supply_chain.py              # Sections 1-7, ~60s
poetry run python scripts/analysis/diagnose_supply_chain.py --full        # + Section 8 inventory deep-dive
poetry run python scripts/analysis/diagnose_supply_chain.py --section 3   # Specific section only
```

Sections: physics validation, executive scorecard, service performance, inventory health, flow efficiency, manufacturing, financial, inventory deep-dive.

**ERP database diagnostic** (runs against PostgreSQL, 52+ questions, 10 sections):
...
poetry run python scripts/analysis/diagnose_erp_database.py --connection "postgresql://user:pass@host:5432/erp_db"
```

Sections: data landscape, physical-financial reconciliation, SCOR source/make/deliver/return, Desmet's triangle, temporal integrity, friction audit, digital thread.
...
src/prism_sim/
├── simulation/
│   ├── orchestrator.py   # Main time-loop controller
│   ├── state.py          # StateManager — vectorized numpy tensors
│   ├── world.py          # World object — nodes, links, products
│   ├── builder.py        # WorldBuilder — constructs World from config
│   ├── demand.py         # POSEngine — consumer sales (promo + seasonality)
│   ├── logistics.py      # LogisticsEngine — bin-packing, shipments, transit
│   ├── mrp.py            # MRPEngine — material requirements planning
│   ├── drp.py            # DRPPlanner — distribution requirements planning
│   ├── transform.py      # TransformEngine — manufacturing physics
│   ├── quirks.py         # QuirkManager — behavioral realism
│   ├── risk_events.py    # RiskEventManager — disruption scenarios
│   ├── monitor.py        # RealismMonitor, PhysicsAuditor
│   ├── warm_start.py     # WarmStartState — load converged state
│   └── writer.py         # StreamingParquetWriter
├── agents/
│   ├── allocation.py     # AllocationAgent — Fair Share, Fill-or-Kill
│   └── replenishment.py  # MinMaxReplenisher — (s,S) policy
├── network/
│   ├── core.py           # Node, Link, Order, Shipment primitives
│   └── recipe_matrix.py  # Vectorized BOM for O(1) MRP
├── product/core.py       # Product, Recipe, ProductCategory
├── config/               # JSON configuration (see below)
└── generators/           # Static world generation (~4,200 nodes)

scripts/
├── erp/                  # ERP export (10 modules → 38 CSV tables)
├── analysis/
...
All simulation parameters are config-driven — no hardcoded values. Configuration files are in `src/prism_sim/config/`:

| File | Purpose |
|---|---|
| `simulation_config.json` | Manufacturing, logistics, quirk parameters |
| `world_definition.json` | Network topology, products, BOMs |
| `cost_master.json` | Unit costs, channel economics, friction rates |
| `benchmark_manifest.json` | Risk scenarios, validation targets |
| `scor_reference.json` | SCOR metric definitions and benchmarks |
