...
class MinMaxReplenisher:
    ...
    def generate_orders(self, day: int, demand_history: np.ndarray) -> List[Order]:
        ...
        for r, c in zip(rows, cols):
            ...
            store_idx = store_indices[r] # Map back to global index/ID? No, r is index into store_indices
            global_store_idx = store_indices[r]
            
            if global_store_idx not in orders_by_store:
                orders_by_store[global_store_idx] = []
            
            p_id = self.state.product_idx_to_id[c]
            orders_by_store[global_store_idx].append(OrderLine(product_id=p_id, quantity=qty))
            
        # Create Order Objects
        order_count = 0
        for store_idx, lines in orders_by_store.items():
            store_id = self.state.node_idx_to_id[store_idx]
