...
class ProductGenerator:
    ...
    def _load_profiles(self) -> None:
        ...
        profiles_config = self.config.get("generation_profiles", {}).get("categories", {})
        ...
        for key, p in profiles_config.items():
            cat = cat_map.get(key)
            if cat:
                self.profiles[cat] = CategoryProfile(
                    category=cat,
                    avg_weight_kg=p["avg_weight_kg"],
                    avg_volume_cc=p["avg_volume_cc"],
                    cases_per_pallet_range=tuple(p["cases_per_pallet_range"]),
                    cost_range=tuple(p["cost_range"]),
                    dim_ratios=tuple(p["dim_ratios"]),
                    run_rate=p["run_rate_cases_per_hour"],
                    changeover=p["changeover_time_hours"]
                )
    ...
    def generate_products(self, n_skus: int = 50) -> list[Product]:
        ...
        if not cats:
            raise ValueError("No category profiles found in configuration")
