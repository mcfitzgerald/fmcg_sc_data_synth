...
59 questions organized across 10 sections probing data validity and utility
from the perspective of veteran SC consultants using SCOR-DS, Desmet's
Triangle, and Mentzer's SC models.

Connects via psycopg to a PostgreSQL database containing 36 normalized tables
(~368M rows) generated by scripts/erp/ from Prism Sim output.

Sections:
  0. Data Landscape (6 Qs)              — Table census, date range, topology, network enrichment
  ...
  3. SCOR Make Process (9 Qs)           — WO→Batch, yield, BOM integrity, PREMIX, variance
  4. SCOR Deliver Process (8 Qs)        — Shipments, fill rate, freight, order pricing
  ...
  8. Friction Layer Audit (8 Qs)        — Controlled noise at expected rates, alias integrity
  9. Digital Thread & Traceability (7 Qs) — reference_id, node_id, names, lead times
...
Usage:
    ...
    poetry run python scripts/analysis/diagnose_erp_database.py --connection "postgresql://postgres:postgres@localhost:5432/erp_db"

Default connection: postgresql://postgres:postgres@localhost:5432/erp_db
Output is automatically tee'd to data/output/diagnostics/diagnose_erp_database_<timestamp>.txt
...
def _f(val: object) -> float:
    """Coerce a psycopg Decimal/int/None to float for arithmetic."""
    if val is None:
        return 0.0
...
def run_section0(conn: psycopg.Connection) -> dict[str, int]:
    """SECTION 0: Data Landscape (Q0.1-Q0.6)."""
    _section_header(0, "DATA LANDSCAPE")
    counts = _verdict_counts()

    # Q0.1 — Table Census
    _q_header("Q0.1", "Table Census — all 36 tables present & populated?")
    # Use actual COUNT(*) instead of pg_stat_user_tables.n_live_tup which is
    # stale (0) after bulk COPY without ANALYZE.
    ...
    print(f"  {'Table':<35} {'Rows':>14}")
    print(f"  {'─' * 35} {'─' * 14}")
    for name, row_count in rows:
        flag = " *EMPTY*" if row_count == 0 else ""
        print(f"  {name:<35} {row_count:>14,}{flag}")
    ...
    # Q0.6 — Network Enrichment — lateral routes + multi-source DCs
    _q_header("Q0.6", "Network Enrichment — lateral RDC↔RDC routes & multi-source DCs")
    with conn.cursor() as cur:
        cur.execute("""
            SELECT COUNT(*) FROM route_segments
            WHERE origin_type = 'rdc' AND destination_type = 'rdc'
        """)
        lateral = cur.fetchone()[0]
        cur.execute("""
            SELECT COUNT(DISTINCT destination_id)
            FROM route_segments
            WHERE origin_type = 'rdc' AND destination_type = 'customer_dc'
            GROUP BY destination_id
            HAVING COUNT(DISTINCT origin_id) >= 2
        """)
        multi_source_dcs = cur.rowcount

    print(f"  Lateral RDC→RDC route segments: {lateral} (expected 12)")
    print(f"  Multi-source DCs (>=2 upstream RDCs): {multi_source_dcs}")

    v_lat = "PASS" if lateral >= 12 else ("WARN" if lateral > 0 else "FAIL")
    v_ms = "PASS" if multi_source_dcs > 0 else "WARN"
    _kpi_row("Lateral Routes", str(lateral), ">=12", v_lat)
    _kpi_row("Multi-source DCs", str(multi_source_dcs), ">0", v_ms)
    counts[v_lat.lower()] += 1
    counts[v_ms.lower()] += 1
...
def run_section1(conn: psycopg.Connection) -> dict[str, int]:
...
def run_section2(conn: psycopg.Connection) -> dict[str, int]:
    ...
    # Match each PO to its first GR at the same plant (avoids Cartesian
    # explosion from the old plant+date proximity join which created 1B+
    # spurious pairs and a meaningless P50 = midpoint of the date window).
    _q_header("Q2.3", "Procurement Lead Time — PO to first GR")
    with conn.cursor() as cur:
        cur.execute("""
            SELECT
                percentile_cont(0.50) WITHIN GROUP (ORDER BY first_gr_lag),
                percentile_cont(0.05) WITHIN GROUP (ORDER BY first_gr_lag),
                percentile_cont(0.95) WITHIN GROUP (ORDER BY first_gr_lag),
                AVG(first_gr_lag),
                COUNT(*)
            FROM (
                SELECT po.id,
                       MIN(gr.receipt_date) - po.order_date as first_gr_lag
            ...
            WHERE first_gr_lag >= 0
        """)
        p50, p05, p95, avg_lt, n_pos = cur.fetchone()

    if p50 is not None:
        print(f"  POs matched to first GR: {n_pos:,}")
        print(f"  Lead time P5/P50/P95: {float(p05):.0f} / {float(p50):.0f} / {float(p95):.0f} days")
        print(f"  Mean: {float(avg_lt):.1f} days")
        v = "PASS" if 0 <= float(p50) <= 7 else ("WARN" if float(p50) <= 15 else "FAIL")
    else:
        print("  No PO→GR matches found")
        v = "FAIL"
    _kpi_row("Median Lead Time", f"{float(p50 or 0):.0f}d", "0-7d", v)
    counts[v.lower()] += 1
...
def run_section3(conn: psycopg.Connection) -> dict[str, int]:
    """SECTION 3: SCOR Make Process (Q3.1-Q3.9)."""
    ...
    # Compare FG cases shipped from plants to store-level demand.
    # Denominator uses only store-targeted orders (JOIN retail_locations)
    # to avoid inflating demand by counting every echelon hop.
    _q_header("Q3.6", "Production/Demand Alignment — FG cases shipped vs store demand")
    with conn.cursor() as cur:
        cur.execute("""
        ...
        cur.execute("""
            SELECT SUM(ol.quantity_cases)
            FROM order_lines ol
            JOIN orders o ON o.id = ol.order_id
            JOIN retail_locations rl ON rl.id = o.retail_location_id
        """)
    ...
    print(f"  Store demand:           {_f(demand_cases) / 1e6:,.1f}M cases")
    print(f"  Ship/Demand ratio: {ratio:.3f}")

    # Plants ship >100% of store demand because buffer stock builds at RDCs/DCs
    v = _traffic(ratio, 0.90, 1.50, 0.50, 2.00)
    _kpi_row("Ship/Demand Ratio", f"{ratio:.3f}", "0.90-1.50", v)
    counts[v.lower()] += 1
    ...
    expected_types = {"finished_good", "bulk_intermediate", "premix"}
    has_all = expected_types.issubset(types_present)
    v = "PASS" if has_all else ("WARN" if "finished_good" in types_present else "FAIL")
    actual_label = "Yes" if has_all else f"Missing: {expected_types - types_present}"
    _kpi_row("All 3 Types Present", actual_label, "fg+bulk+premix", v)
    counts[v.lower()] += 1

    # Q3.8 — PREMIX / 3-Level BOM Validation
    _q_header("Q3.8", "PREMIX Sub-Intermediates & Variable BOM Depth")
    with conn.cursor() as cur:
        # Premix products in bulk_intermediates
        cur.execute("SELECT COUNT(*) FROM bulk_intermediates WHERE bom_level = 2")
        n_premix_products = cur.fetchone()[0]
        # Premix formulas
        cur.execute("SELECT COUNT(*) FROM formulas WHERE bom_level = 2")
        n_premix_formulas = cur.fetchone()[0]
        # Premix batches
        cur.execute("SELECT COUNT(*) FROM batches WHERE product_type = 'premix'")
        n_premix_batches = cur.fetchone()[0]
        # Formula depth: bom_level=1 formulas that consume premix (ingredient_id in bulk_intermediates with bom_level=2)
        cur.execute("""
            SELECT COUNT(DISTINCT fi.formula_id)
            FROM formula_ingredients fi
            JOIN formulas f ON f.id = fi.formula_id AND f.bom_level = 1
            JOIN bulk_intermediates bi ON bi.id = fi.ingredient_id AND bi.bom_level = 2
        """)
        n_bl1_using_premix = cur.fetchone()[0]

    print(f"  PREMIX products (bom_level=2): {n_premix_products} (expected 13)")
    print(f"  PREMIX formulas (bom_level=2): {n_premix_formulas} (expected 13)")
    print(f"  PREMIX batches: {n_premix_batches:,}")
    print(f"  BOM-level-1 formulas consuming premix: {n_bl1_using_premix}")

    v = "PASS" if n_premix_products >= 13 and n_premix_formulas >= 13 and n_premix_batches > 0 else "FAIL"
    _kpi_row("PREMIX Products", str(n_premix_products), ">=13", v)
    counts[v.lower()] += 1

    v2 = "PASS" if n_bl1_using_premix > 0 else "WARN"
    _kpi_row("3-Level BOM Chains", str(n_bl1_using_premix), ">0", v2)
    counts[v2.lower()] += 1

    # Q3.9 — Batch Ingredient Variance (planned vs actual)
    _q_header("Q3.9", "Batch Ingredient Variance — planned vs actual (+/-3%)")
    with conn.cursor() as cur:
        # formula_ingredients.quantity_kg = kg of ingredient per production unit (case).
        # batch_ingredients.quantity_kg = total kg consumed in the batch.
        # batches.quantity_kg = number of units (cases) produced.
        # Expected = fi.quantity_kg * b.quantity_kg; variance from +/-3% recording noise.
        cur.execute("""
            SELECT
                COUNT(*) as n_rows,
                AVG(ABS(bi.quantity_kg - fi.quantity_kg * b.quantity_kg)
                    / NULLIF(fi.quantity_kg * b.quantity_kg, 0)) as avg_pct_var,
                MAX(ABS(bi.quantity_kg - fi.quantity_kg * b.quantity_kg)
                    / NULLIF(fi.quantity_kg * b.quantity_kg, 0)) as max_pct_var
            FROM batch_ingredients bi
            JOIN batches b ON b.id = bi.batch_id
            JOIN formulas f ON f.id = b.formula_id
            JOIN formula_ingredients fi ON fi.formula_id = f.id AND fi.ingredient_id = bi.ingredient_id
            WHERE fi.quantity_kg > 0
        """)
        var_row = cur.fetchone()

    n_matched = var_row[0] if var_row else 0
    avg_var = _f(var_row[1]) * 100 if var_row and var_row[1] else 0
    max_var = _f(var_row[2]) * 100 if var_row and var_row[2] else 0

    print(f"  Matched batch_ingredients ↔ formula_ingredients: {n_matched:,}")
    print(f"  Average |variance|: {avg_var:.2f}%")
    print(f"  Max |variance|: {max_var:.2f}%")

    # Variance may be ~0% (disabled in sim config) or 1-5% (enabled).
    # Both are valid; only FAIL if clearly broken (>10%).
    v = "PASS" if avg_var <= 5 else ("WARN" if avg_var <= 10 else "FAIL")
    _kpi_row("Avg Batch Variance", f"{avg_var:.2f}%", "<=5%", v)
    counts[v.lower()] += 1
...
def run_section4(conn: psycopg.Connection) -> dict[str, int]:
    """SECTION 4: SCOR Deliver Process (Q4.1-Q4.8)."""
    ...
    print("  (Only dc_to_store shipments generate AR invoices, ~15-25%)")

    v = _traffic(ar_pct, 10, 30, 5, 40)
    _kpi_row("AR/Shipment Rate", f"{ar_pct:.1f}%", "15-25%", v)
    counts[v.lower()] += 1
    ...
    # Q4.5 — Aggregate Fill Rate (dc_to_store deliveries vs store demand)
    # Numerator: cases actually shipped dc→store.
    # Denominator: cases ordered by stores (orders whose target is a
    # retail_location).  The orders table uses globally-unique location PKs
    # from the unified IdMapper, so JOIN retail_locations filters correctly.
    ...
    print(f"  Store demand:       {float(ordered_cases):,.0f}")
    print(f"  Fill rate: {fill_rate:.1f}%")

    v = _traffic(fill_rate, 85, 100, 50, 100)
    _kpi_row("Fill Rate", f"{fill_rate:.1f}%", ">85%", v)
    counts[v.lower()] += 1
    ...
    # Q4.8 — Order Line Unit Price Population
    _q_header("Q4.8", "Order Line Unit Price — populated (non-zero)?")
    with conn.cursor() as cur:
        cur.execute("SELECT COUNT(*) FROM order_lines")
        total_ol = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM order_lines WHERE unit_price > 0")
        priced = cur.fetchone()[0]

    pct_priced = _pct(priced, total_ol)
    print(f"  Total order lines: {total_ol:,}")
    print(f"  With unit_price > 0: {priced:,} ({pct_priced:.1f}%)")

    v = "PASS" if pct_priced > 95 else ("WARN" if pct_priced > 50 else "FAIL")
    _kpi_row("Priced Lines", f"{pct_priced:.1f}%", ">95%", v)
    counts[v.lower()] += 1
...
def run_section5(conn: psycopg.Connection) -> dict[str, int]:
...
def run_section6(conn: psycopg.Connection) -> dict[str, int]:
    """SECTION 6: Desmet's Triangle (Q6.1-Q6.7)."""
    _section_header(6, "DESMET'S TRIANGLE — Service vs Cost vs Cash")
    counts = _verdict_counts()

    # Q6.1 — Service: Fill Rate + Returns (store demand only)
    ...
    ok = fill_rate > 85 and return_rate < 5
    v = "PASS" if ok else ("WARN" if fill_rate > 50 else "FAIL")
    _kpi_row("Service Score", f"{fill_rate:.1f}% / {return_rate:.1f}%", ">85% / <5%", v)
    counts[v.lower()] += 1
    ...
    # Q6.6 — Cash: DIO (all FG inventory, sampled weekly)
    # Include ALL echelons with FG stock: plant, rdc, customer_dc, store.
    # The inventory table only has sku_id (FG products), so plant rows are
    # already FG-only (RM goes through ingredient tables, not inventory).
    # Use per-SKU cost_per_case for weighted valuation.
    _q_header("Q6.6", "Cash: DIO — Days Inventory Outstanding (all FG)")
    with conn.cursor() as cur:
        # Avg weekly FG inventory value across all echelons
        cur.execute("""
            ...
            FROM (
                ...
                WHERE MOD(i.day, 7) = 0
                  AND i.location_type IN ('plant', 'rdc', 'customer_dc', 'store')
...
def run_section7(conn: psycopg.Connection) -> dict[str, int]:
...
def run_section8(conn: psycopg.Connection) -> dict[str, int]:
    """SECTION 8: Friction Layer Audit (Q8.1-Q8.8)."""
    ...
    with conn.cursor() as cur:
        cur.execute("""
            SELECT COUNT(*), COUNT(CASE WHEN gr_id IS NULL THEN 1 END)
            FROM ap_invoices
        """)
    ...
    # Q8.8 — SKU Alias Chain Integrity — depth-1, no cycles, FKs valid
    _q_header("Q8.8", "SKU Alias Chain Integrity — depth-1, no cycles, valid FKs")
    with conn.cursor() as cur:
        # All -OLD SKUs have supersedes_sku_id pointing to a valid SKU
        cur.execute("""
            SELECT COUNT(*) FROM skus
            WHERE sku_code LIKE '%-OLD' AND supersedes_sku_id IS NULL
        """)
        old_no_link = cur.fetchone()[0]
        # No cycles: target of supersedes_sku_id should NOT itself have supersedes_sku_id (depth-1)
        cur.execute("""
            SELECT COUNT(*)
            FROM skus s1
            JOIN skus s2 ON s1.supersedes_sku_id = s2.id
            WHERE s2.supersedes_sku_id IS NOT NULL
        """)
        depth_gt_1 = cur.fetchone()[0]
        # Target SKU is active
        cur.execute("""
            SELECT COUNT(*)
            FROM skus s1
            JOIN skus s2 ON s1.supersedes_sku_id = s2.id
            WHERE s2.is_active = false
        """)
        target_inactive = cur.fetchone()[0]

    print(f"  -OLD SKUs without supersedes_sku_id: {old_no_link} (expected 0)")
    print(f"  Chains deeper than 1 hop: {depth_gt_1} (expected 0)")
    print(f"  Aliases pointing to inactive target: {target_inactive} (expected 0)")

    v = "PASS" if old_no_link == 0 and depth_gt_1 == 0 else "FAIL"
    _kpi_row("Alias Chain Integrity", f"orphan={old_no_link},depth>1={depth_gt_1}", "0/0", v)
    counts[v.lower()] += 1
...
def run_section9(conn: psycopg.Connection) -> dict[str, int]:
    """SECTION 9: Digital Thread & Traceability (Q9.1-Q9.7)."""
    ...
    # Q9.6 — Real Entity Names — ingredients + suppliers have descriptive names
    _q_header("Q9.6", "Real Entity Names — descriptive ingredient/supplier names")
    with conn.cursor() as cur:
        # Ingredients: check names aren't just codes (e.g., "Ingredient-001")
        cur.execute("""
            SELECT COUNT(*) FROM ingredients
            WHERE name ~ '^[A-Z][a-z]' AND LENGTH(name) > 10
        """)
        real_ing_names = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM ingredients")
        total_ing = cur.fetchone()[0]

        # Suppliers: check names aren't just codes
        cur.execute("""
            SELECT COUNT(*) FROM suppliers
            WHERE name ~ '^[A-Z][a-z]' AND LENGTH(name) > 10
              AND supplier_code NOT LIKE '%-ALT'
        """)
        real_sup_names = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM suppliers WHERE supplier_code NOT LIKE '%-ALT'")
        total_sup = cur.fetchone()[0]

    ing_pct = _pct(real_ing_names, total_ing)
    sup_pct = _pct(real_sup_names, total_sup)
    print(f"  Ingredients with real names: {real_ing_names}/{total_ing} ({ing_pct:.0f}%)")
    print(f"  Suppliers with real names: {real_sup_names}/{total_sup} ({sup_pct:.0f}%)")

    v = "PASS" if ing_pct > 90 and sup_pct > 90 else ("WARN" if ing_pct > 50 else "FAIL")
    _kpi_row("Real Names", f"ing={ing_pct:.0f}%,sup={sup_pct:.0f}%", ">90%", v)
    counts[v.lower()] += 1

    # Q9.7 — Lead Time Diversity — range across sourcing tiers
    _q_header("Q9.7", "Lead Time Diversity — 5-72 day range")
    with conn.cursor() as cur:
        cur.execute("""
            SELECT MIN(lead_time_days), MAX(lead_time_days),
                   ROUND(AVG(lead_time_days)::numeric, 1),
                   COUNT(DISTINCT lead_time_days)
            FROM supplier_ingredients
        """)
        lt_min, lt_max, lt_avg, lt_distinct = cur.fetchone()

    print(f"  Lead time range: {lt_min}-{lt_max} days")
    print(f"  Average: {lt_avg} days")
    print(f"  Distinct values: {lt_distinct}")

    has_range = lt_max - lt_min >= 30 if lt_min is not None and lt_max is not None else False
    v = "PASS" if has_range and lt_distinct >= 10 else ("WARN" if has_range else "FAIL")
    _kpi_row("LT Range", f"{lt_min}-{lt_max}d", ">=30d spread", v)
    counts[v.lower()] += 1
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="ERP Database Diagnostic — 59 Questions, 10 Sections"
    )
    parser.add_argument(
        "--connection", type=str,
        default="postgresql://postgres:postgres@localhost:5432/erp_db",
        help="PostgreSQL connection string (default: postgresql://postgres:postgres@localhost:5432/erp_db)",
    )
    parser.add_argument(
    ...
    print("=" * WIDTH)
    print("  ERP DATABASE DIAGNOSTIC (v0.93.0)".center(WIDTH))
    print("  59 Questions — A Consultant's Engagement Workbook".center(WIDTH))
    print("=" * WIDTH)
    ...
    for sec_num, sec_name, runner_fn in section_runners:
        if args.section not in (-1, sec_num):
            continue
        print(f"\n>>> Running Section {sec_num}: {sec_name}...", flush=True)
        t_sec = time.time()
        try:
